...
def createConfig(args, dataset):
    ...
    else: config.Data.inputDataset = dataset
    if args.use_parent: config.Data.useParent = True
...
def main():
    ...
    parser.add_argument('--send-python',
                        action='store_true', default=False,
    ...
    parser.add_argument('--use-parent',
                        action='store_true', default=False, 
    ...
    parser.add_argument('--set-input-files',
                        action='store_true', default=False,
    ...
    if args.set_input_files:
     ...
     with open(args.inputfile) as inputfile:
        for l in inputfile:
            ...
            if len(dataset)==0: dataset = [s for s in l.split() if '/AOD' in s]
            if len(dataset)==0: dataset = [s for s in l.split() if '/GEN' in s]
            if len(dataset)==0: dataset = [s for s in l.split() if '/RAW' in s]
            if len(dataset)==0:
             print("The dataset is not of the following types: /MINIAOD, /AOD, /GEN, /RAW")
             sys.exit()
            dataset=dataset[0]
