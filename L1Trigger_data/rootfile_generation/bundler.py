...
def temp_file_creator(config):
    ...
    for root_file in root_list:
        ...
        f = open(temp_file, "a")
        f.write("[Caution] This is a temporary file that the worker uses to pick its jobs. It will be deleted automatically. Please do not delete this file !!!")
        f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config-file', type=str, required=True, help='Path to the configuration YAML file containing the master config')
    parser.add_argument('--job_name', type=str,default="Ntupliser")
    parser.add_argument('--submit_flag',default=True)  # This is just added for making the CI control easier in practice this should not be turned off
    ...
    for name in list(INPATHS.keys()):
        ...
        job_script_path = os.path.join(outpath_afs,"job.sh") # Since HTCondor Jobs cannot be submitted from EOS
...
"""
        ...
        submit_script_path = os.path.join(outpath_afs,"submit.sub") # Since HTCondor Jobs cannot be submitted from EOS
